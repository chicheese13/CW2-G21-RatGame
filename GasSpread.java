...
public class GasSpread extends RenderObject {
    ...
    public boolean checkParent(Position position) {
        if (this.parentGas.getObjectPosition()[0]
                .doubleValue() == position.getPosition()[0].doubleValue()
                && this.parentGas.getObjectPosition()[1]
                        .doubleValue() == position.getPosition()[1]
                                .doubleValue()) {
            return false;
        }
        return true;
    ...
    public void tick() {
        ...
        if (this.parentGas.getTick() >= STOP_SPREAD_INTERVAL) {
            ...
            if (this.currentLevel.tileAvailable(this.getObjectPosition()[0],
                    this.getObjectPosition()[1], 'N')) {
                // create new position y - 1

                Position newPosition = new Position(this.getObjectPosition()[0],
                        this.getObjectPosition()[1]
                                .subtract(new BigDecimal("1")));
                if (this.currentLevel.checkGas(newPosition)
                        && checkParent(newPosition)) {
                    this.currentLevel.spawnTempTile(new GasSpread(newPosition,
                            this.currentLevel, this.parentGas));
                }
            }
            // check east
            if (this.currentLevel.tileAvailable(this.getObjectPosition()[0],
                    this.getObjectPosition()[1], 'E')) {
                Position newPosition = new Position(
                        this.getObjectPosition()[0].add(new BigDecimal("1")),
                        this.getObjectPosition()[1]);
            ...
            // check west
            if (this.currentLevel.tileAvailable(this.getObjectPosition()[0],
                    this.getObjectPosition()[1], 'W')) {
                Position newPosition = new Position(
                        this.getObjectPosition()[0]
                                .subtract(new BigDecimal("1")),
                        this.getObjectPosition()[1]);
                if (this.currentLevel.checkGas(newPosition)
                        && checkParent(newPosition)) {
