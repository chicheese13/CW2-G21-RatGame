...
public abstract class NormalRat extends Rat {
     ...
     */
    // maybe change, extra feature mother rat becomes sterile whilst pregnant
    // may result in sterile baby rats.
    ...
    /**
     * The gender of the rat, true meaning a male rat, false meaning a female
     * rat.
     */
    ...
    /**
     * Calculates a new position based on the current position and a new speed
     * value, this is to prevent the new speed value from incrimenting the
     ...
     */
    public Position recallibratePosition(Position position,
            BigDecimal newSpeed) {
        ...
        // adds the returned value from the round to multiple function to the
        // floored values of x and y if need be
        // returns a new position that will work the new speed.

        BigDecimal flooredX = position.getPosition()[0].setScale(0,
                RoundingMode.DOWN);
        BigDecimal flooredY = position.getPosition()[1].setScale(0,
                RoundingMode.DOWN);
    ...
    /**
     * This takes in a value that needs to be changed to a multiple of the new
     * rat speed value.
     * 
     * @param change, decimal value that needs to be rounded to the nearest
     * multiple.
     * @param speed, the decimal value that the change needs to be rounded to a
     * multiple of.
     */
    public BigDecimal roundToMutliple(BigDecimal change, BigDecimal speed) {
        ...
        // divide the change by the speed, round the outcome and then multiply
        // by speed
        BigDecimal roundedDivision = (multipliedChange.divide(multipliedSpeed))
                .setScale(0, RoundingMode.HALF_UP);
        // this rounds the change value to the nearest multiple of speed
        BigDecimal output = (roundedDivision.multiply(multipliedSpeed))
